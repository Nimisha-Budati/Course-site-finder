...
def add_review(request):
    if request.method == "POST":
        print("DEBUG → Request POST:", request.POST)  # REMOVE later
...
def save_course(request):
    if request.method == "POST":
        ...
        exists = SavedCourse.objects.filter(
            user=request.user,
            course_id=course_id,
            course_url=url
        ).exists()
